...
void test(uint32 numPages, uint32 numInitPages){
	...
	ptr[i*PAGE_SIZE] = 'A';
	if((i>15350) && (i<15400))
		kprintf("i:%d c:%c addr:%d\n",i,ptr[i*PAGE_SIZE],&ptr[i*PAGE_SIZE]);
    }
    ...
    // read data
    char c = 0;
	// for(int i=15300;i<15400;i++){
	// 	kprintf("i:%d c:%c addr:%x\n",i,ptr[i*PAGE_SIZE],&ptr[i*PAGE_SIZE]);
	// }
    ...
    process_info(currpid);
#endif
	kprintf("alloc:%d used:%d\n",allocated_virtual_pages(currpid),used_ffs_frames(currpid));
    if(allocated_virtual_pages(currpid)!= PREALLOCATED_PAGES || used_ffs_frames(currpid)!=0){
	error = 1;
    }
	kprintf("error 2:%d\n",error);
