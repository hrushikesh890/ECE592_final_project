...
int search_LRU_FFS()
{
	...
	for (i = 0; i < 16*1024; i++)
	{
		if (ipt[i].proc_pid != 0)
		{
			...
			pt_t* pt = (pt_t*)pd[ipt[i].pd_offset].pd_base;
			if (pt[ipt[i].pt_offset].pt_acc == 0)
			{
				return i;
			}
		}
...
{
	//kprintf("Reached swap\n");
	intmask	mask;
	write_cr3((unsigned long)PT_START);
	mask = disable();
	pd_t* pd = (pd_t*) proctab[currpid].pdbr;
	pt_t* pt = (pt_t*) (pd[pd_offset].pd_base << 12);
	inv_pt* ipt = (inv_pt *)(INV_TABLE_START);
	int id;
	pid32 ppid;
	uint32 addr;

	kprintf("swap frames :%d\n",MAX_SWAP_SIZE-free_swap_pages());
	write_cr3((unsigned long)PT_START);

	//kprintf("pd:%x pt:%x pd_off:%d pt_off:%d swp:%d\n",pd,pt,pd_offset,pt_offset,pt[pt_offset].pt_swap);
	...
	if (pt[pt_offset].pt_swap == 1) // already present in swap space
	{
		...
		if (id != -1)
		{
			...
			if (pt1[ipt[id].pt_offset].pt_swap == 1)
			{
				...
				pt[pt_offset].pt_base = pt1[ipt[id].pt_offset].pt_base; // save the new pt to the ffs
				pt1[ipt[id].pt_offset].pt_base = ipt[id].swp_addr;
			...
			else
			{
				...
				pt[pt_offset].pt_base = pt1[ipt[id].pt_offset].pt_base; // save the new pt to the ffs
				pt1[ipt[id].pt_offset].pt_base = (uint32) getswpmem(4096);
				write_cr3((unsigned long)PT_START);
				pt[pt_offset].pt_pres = 1;
				pt1[ipt[id].pt_offset].pt_pres = 0;
	...
	{
		//kprintf("In else pd:%x pt:%x pd_off:%d pt_off:%d swp:%d\n",pd,pt,pd_offset,pt_offset,pt[pt_offset].pt_swap);
...
void	swpmeminit(void) {
	...
	//maxheap already initialized in i386.c
       ...
       //pdtlist.mnext = (struct memblk *)PT_START;
       	...
       	swplist.mnext = memptr = (struct memblk *) roundmb(minheap);
